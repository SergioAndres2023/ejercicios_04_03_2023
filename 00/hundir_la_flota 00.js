...
function iashot() {
  // Generar un número aleatorio para la columna y la fila
  const column = (Math.floor(Math.random() * 10));
  const row = (Math.floor(Math.random() * 10));

  // Obtener el símbolo en la posición correspondiente del tablero del jugador
  const symbol = myBoard[row][column];

  // Verificar si no hay ningún objeto en esa posición
  if (!symbol) {
    // Si es así, establecer esa posición en el tablero de la IA como "agua"
    IAboard [row][column] = water;
  } else {
    // De lo contrario, establecer esa posición en el tablero de la IA como "impacto"
    IAboard [row][column] = hit;
  }
}
